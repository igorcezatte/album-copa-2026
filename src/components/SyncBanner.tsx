...
export function SyncBanner() {
  ...
  return (
    <div
      className="mx-4 mb-4 rounded-2xl border border-white/10 bg-white/5 px-4 py-3 flex items-center gap-3 animate-fade-in corner-cut corner-cut-sm"
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[10px] font-mono tracking-wider text-white/40 leading-tight mt-1">
          Crie conta grátis ou baixe um backup
        </p>
      </div>
      <div className="flex items-center gap-2 flex-shrink-0">
        <button
          onClick={() => signIn('google', { callbackUrl: '/' })}
          className="text-[10px] font-mono font-black tracking-widest uppercase text-copa-gold bg-copa-gold/10 px-2.5 py-1.5 rounded-lg active:scale-95 transition-transform whitespace-nowrap"
        >
          Entrar
        </button>
        <button
