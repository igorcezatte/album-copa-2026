...
export default function StatsPage() {
  ...
  return (
    <div className="px-4 md:px-6 pt-6 pb-4 animate-fade-in">
      <p className="text-[10px] lg:text-[11px] text-white/30 font-mono tracking-[0.22em] uppercase">Dashboard</p>
      <h1 className="text-2xl lg:text-4xl font-display font-black text-white tracking-tight uppercase mb-5 lg:mb-7 mt-0.5">Estatísticas</h1>

      {/* Resumo geral — 3+2 em mobile, 5 numa linha em desktop */}
      <div className="grid grid-cols-6 md:grid-cols-5 gap-2 md:gap-3 lg:gap-4 mb-6 lg:mb-8">
      ...
      {/* Progresso por grupo */}
      <h2 className="text-xs lg:text-sm font-display font-bold text-white/40 uppercase tracking-[0.2em] mb-3 lg:mb-4 flex items-baseline gap-2">
        <span className="font-mono text-white/25" aria-hidden>—</span>
        Por grupo
      </h2>
      <div className="space-y-2 md:grid md:grid-cols-2 md:gap-x-8 md:gap-y-2 lg:gap-y-3 md:space-y-0 mb-6 lg:mb-8">
        {groupStats.map((g) => (
          <div key={g.group} className="flex items-center gap-3">
            <span
              className="text-sm lg:text-lg font-display font-black tracking-tight w-5 lg:w-6 text-center flex-shrink-0"
            ...
            <span className="text-[11px] lg:text-sm font-mono font-bold tracking-wider text-white/40 w-9 lg:w-11 text-right flex-shrink-0">
              {g.pct}%
            </span>
          </div>
        ))}
      ...
      <div className="space-y-1.5 md:grid md:grid-cols-2 xl:grid-cols-3 md:gap-x-4 md:gap-y-1.5 lg:gap-y-2 md:space-y-0">
        {sorted.map((team, i) => {
          ...
          return (
            <div
              key={team.code}
              className="flex items-center gap-2.5 lg:gap-3 rounded-xl px-3 py-2 lg:px-4 lg:py-2.5 border border-white/5"
              style={{ background: `linear-gradient(90deg, ${color}10 0%, var(--copa-card) 100%)` }}
            >
              <span className="text-[11px] lg:text-xs font-mono font-bold tracking-wider text-white/30 w-5 flex-shrink-0">
                {String(i + 1).padStart(2, '0')}
              </span>
              <Flag code={team.flagCode} size="xs" grayscale={percentage === 0} />
              <span className="text-xs lg:text-sm font-display font-bold tracking-wide uppercase text-white flex-1 truncate">{team.name}</span>
              <div className="w-16 lg:w-24 flex-shrink-0">
                <ProgressBar value={team.collected} total={team.total} color={color} height="xs" />
              </div>
              <span
                className="text-xs lg:text-sm font-display font-black tracking-tight w-9 lg:w-11 text-right flex-shrink-0"
...
function StatCard({ value, label, color, className }: { value: number | string; label: string; color: string; className?: string }) {
  return (
    <div
      className={`rounded-2xl p-3 lg:p-5 border border-white/5 text-center corner-cut corner-cut-sm ${className ?? ''}`}
      style={{
        background: `linear-gradient(145deg, ${color}15 0%, var(--copa-card) 100%)`,
        ['--cut-accent' as string]: `${color}aa`,
      } as React.CSSProperties}
    >
      <p className="text-3xl lg:text-5xl font-display font-black leading-none tracking-tight" style={{ color }}>{value}</p>
      <p className="text-[9px] lg:text-[11px] text-white/40 mt-1.5 lg:mt-2.5 font-mono tracking-widest uppercase">{label}</p>
    </div>
  )
}
